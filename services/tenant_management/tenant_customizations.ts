...
router.post('/branding', async (req: Request, res: Response) => {
    ...
    if (!tenantId || !branding) {
        return res.status(400).json({ error: 'Tenant ID and branding details are required' });
    }
...
router.post('/workflows', async (req: Request, res: Response) => {
    ...
    if (!tenantId || !workflows) {
        return res.status(400).json({ error: 'Tenant ID and workflows are required' });
    }
...
router.post('/notifications', async (req: Request, res: Response) => {
    ...
    if (!tenantId || !notifications) {
        return res.status(400).json({ error: 'Tenant ID and notification preferences are required' });
    }
