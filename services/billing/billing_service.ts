...
// Subscription plans
const plans = [
    { id: 'basic', name: 'Basic Plan', price: 1000, features: ['Feature A', 'Feature B'] },
    { id: 'premium', name: 'Premium Plan', price: 2000, features: ['Feature A', 'Feature B', 'Feature C'] },
];
...
fastify.post<{ Body: CreateSubscriptionBody }>('/subscriptions', async (request, reply) => {
    ...
    try {
        ...
        reply.status(201).send({ message: 'Subscription created successfully', subscription });
    } catch (error: any) {
...
fastify.delete<{ Params: ParamsWithId }>('/subscriptions/:id', async (request, reply) => {
    const { id } = request.params;

    try {
        // Cancel the subscription in Stripe
        const cancellation = await stripe.subscriptions.del(id);
        reply.status(200).send({ message: 'Subscription cancelled successfully', cancellation });
    } catch (error: any) {
        reply.status(500).send({ error: error.message });
    }
});
...
fastify.get<{ Params: ParamsWithCustomerId }>('/usage/:customerId', async (request, reply) => {
    const { customerId } = request.params;

    if (!usageRecords[customerId]) {
        return reply.status(404).send({ error: 'No usage records found for this customer' });
    }
...
fastify.post<{ Body: CreateInvoiceBody }>('/invoices', async (request, reply) => {
    ...
    try {
        ...
        reply.status(201).send({ message: 'Invoice generated successfully', invoice });
    } catch (error: any) {
        reply.status(500).send({ error: error.message });
    }
});
...
fastify.post<{ Body: AddDiscountBody }>('/discounts', async (request, reply) => {
...
fastify.post<{ Body: CreateSubscriptionBody & { discountCode?: string } }>('/subscriptions', async (request, reply) => {
...
fastify.post<{ Body: CreateSubscriptionBody }>('/subscriptions/trial', async (request, reply) => {
    const { customerId, planId, paymentMethodId } = request.body;
    ...
    try {
        ...
        reply.status(201).send({ message: 'Trial subscription created successfully', subscription });
    } catch (error: any) {
        reply.status(500).send({ error: error.message });
    }
});
