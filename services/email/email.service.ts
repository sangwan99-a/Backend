...
fastify.addHook('onRequest', async (request, reply) => {
  ...
  ad.authenticate(username, password, (err: Error | null, auth: boolean) => {
    if (err) {
      reply.code(500).send({ error: 'Authentication error', details: err.message });
      return;
    }
...
fastify.register(fastifyOauth2, {
  ...
  credentials: {
    ...
    auth: {
      tokenHost: process.env.OAUTH_TOKEN_HOST || 'https://your-identity-provider.com',
      authorizePath: process.env.OAUTH_AUTHORIZE_PATH || '/oauth/authorize',
      tokenPath: process.env.OAUTH_TOKEN_PATH || '/oauth/token',
    },
  },
  startRedirectPath: '/login',
  callbackUri: process.env.OAUTH_CALLBACK_URI || 'http://localhost:3010/callback',
});
...
fastify.addHook('onRequest', async (request, reply) => {
  try {
    ...
    reply.code(401).send({ error: 'Invalid token', details: (err as Error).message });
  }
});
...
fastify.post('/upload', { preHandler: upload.single('file') }, async (request, reply) => {
  ...
  try {
    ...
    reply.send({ status: 'File uploaded successfully', file: file.originalname });
  } catch (err) {
    reply.code(400).send({ error: 'File upload failed', details: (err as Error).message });
  }
});
...
function tenantContextMiddleware(req: Request, res: Response, next: NextFunction) {
