...
// Plugin monetization
const pluginMonetization: Record<string, { price: number; revenueShare: number }> = {
...
fastify.post<{ Body: SubmitPluginBody }>('/plugins', async (request, reply) => {
    ...
    plugins[name] = { version, description, developer, code, createdAt: new Date() };
    reply.status(201).send({ message: 'Plugin submitted successfully' });
});

// API to update a plugin
fastify.put<{ Params: ParamsWithName; Body: UpdatePluginBody }>('/plugins/:name', async (request, reply) => {
    ...
    plugins[name] = { ...plugins[name], version, description, code, updatedAt: new Date() };
...
fastify.post<{ Params: ParamsWithName; Body: AddChangelogBody }>('/plugins/:name/changelog', async (request, reply) => {
    const { name } = request.params;
...
fastify.post('/tenants/:tenantId/plugins/:name/install', async (request, reply) => {
    ...
    reply.status(200).send({ message: `Plugin '${name}' installed for tenant '${tenantId}'` });
});
...
fastify.patch('/tenants/:tenantId/plugins/:name', async (request, reply) => {
    ...
    reply.status(200).send({ message: `Plugin '${name}' ${enabled ? 'enabled' : 'disabled'} for tenant '${tenantId}'` });
});
