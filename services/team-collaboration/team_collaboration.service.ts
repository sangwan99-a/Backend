...
const consumer = kafka.consumer({ groupId: 'team-collaboration-service-group' });
...
const consumeEvents = async () => {
  ...
  await consumer.subscribe({ topic: 'team-collaboration-events', fromBeginning: true });
  await consumer.run({
...
const start = async () => {
  try {
    ...
    await consumeEvents();
    await httpServer.listen(parseInt(process.env.PORT || '3014', 10), '0.0.0.0');
